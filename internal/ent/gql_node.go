...
func (c *Client) noder(ctx context.Context, table string, id string) (Noder, error) {
	switch table {
	case cabinet.Table:
		query := c.Cabinet.Query().
			Where(cabinet.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, cabinetImplementors...); err != nil {
				return nil, err
			}
		}
...
func (c *Client) noders(ctx context.Context, table string, ids []string) ([]Noder, error) {
	...
	case cabinet.Table:
		query := c.Cabinet.Query().
			Where(cabinet.IDIn(ids...))
		query, err := query.CollectFields(ctx, cabinetImplementors...)
		if err != nil {
			return nil, err
		}

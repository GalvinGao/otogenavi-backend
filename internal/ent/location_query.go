...
type LocationQuery struct {
	...
	withNamedCabinets map[string]*CabinetQuery
...
func (lq *LocationQuery) QueryCabinets() *CabinetQuery {
	query := (&CabinetClient{config: lq.config}).Query()
...
func (lq *LocationQuery) Clone() *LocationQuery {
	...
	return &LocationQuery{
		...
		withCabinets: lq.withCabinets.Clone(),
...
func (lq *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if query := lq.withCabinets; query != nil {
		if err := lq.loadCabinets(ctx, query, nodes,
			func(n *Location) { n.Edges.Cabinets = []*Cabinet{} },
			func(n *Location, e *Cabinet) { n.Edges.Cabinets = append(n.Edges.Cabinets, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range lq.withNamedCabinets {
		if err := lq.loadCabinets(ctx, query, nodes,
			func(n *Location) { n.appendNamedCabinets(name) },
			func(n *Location, e *Cabinet) { n.appendNamedCabinets(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (lq *LocationQuery) loadCabinets(ctx context.Context, query *CabinetQuery, nodes []*Location, init func(*Location), assign func(*Location, *Cabinet)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(cabinet.FieldLocationID)
	}
	query.Where(predicate.Cabinet(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.CabinetsColumn), fks...))
	}))
...
// WithNamedCabinets tells the query-builder to eager-load the nodes that are connected to the "cabinets"
...
func (lq *LocationQuery) WithNamedCabinets(name string, opts ...func(*CabinetQuery)) *LocationQuery {
	...
	if lq.withNamedCabinets == nil {
		lq.withNamedCabinets = make(map[string]*CabinetQuery)
	}
	lq.withNamedCabinets[name] = query
	return lq
}
